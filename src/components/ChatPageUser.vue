...
<template>
    <div class="chat-page">
        ...
        <ul>
            <li
            :class="{ active: target_user_id === 'admin1' }"
            >
            管理员
            </li>
        </ul>
  ...
  // chatpageuser.vue - Only the script section needs modification
  ...
  import { getChatInstance } from '@/services/IMService';
  ...
  const target_user_id = ref('admin1');
  ...
  const loadHistoryMessages = async () => {
    try {
      ...
      await nextTick(); // 等待 DOM 更新完成后再滚动
